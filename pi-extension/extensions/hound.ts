...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    name: "web_search",
    label: "Web Search",
    description: "Keyless web search (no API key, no account). 10 backends in parallel (ddg,brave,mojeek,yahoo,yandex,startpage,google,qwant + opt-in wikipedia,grokipedia), neural-reranked + cross-backend consensus. Returns URLs + ranking, NOT content. \n\nWORKFLOW: Search -> web_fetch the 1-2 best results (fetch_relevance=high first). Use focus='your question' on each fetch to extract only relevant paragraphs and save tokens. Use urls=[...] to bulk-fetch multiple results in one call. \n\nEFFICIENCY: For multi-fact or comparison questions (e.g. 'X across models/years'), do ONE broad search, not one per fact. One good comparison source answers 10 sub-questions in a single fetch with focus=. If results miss something, try related_queries or rephrase ONCE, not N searches per sub-fact. \n\nANTI-PATTERN: Don't search for something you already have a URL for - use web_fetch with focus= instead. NEVER answer from snippets alone - always fetch the page, but 1-2 targeted fetches is enough, not every result. \n\nFILTERS (in options): site='domain.com' restricts to one domain. exclude_sites=['pinterest.com'] removes noise. freshness='day|week|month|year' for time-sensitive queries. page=0-10 for pagination. location/language/region for geo. \n\nRESULT FIELDS: relevance_score (0-1), fetch_relevance (high/med/low - fetch high first), engines_consensus (how many independent indexes returned this URL - higher = more authoritative). related_queries can suggest better search terms.",
    promptSnippet: "web_search(query) - keyless search across 10 backends. Don't search when you have a URL - web_fetch with focus= instead. After search, web_fetch high-relevance results with focus='your question'.",
    parameters: Type.Object({
