...
class ResponseModel(BaseModel):
    ...
    is_truncated: bool = Field(default=False, description="True=use offset for more")
...
class MasterFetchServer:
    ...
    def serve(self, http: bool = False, host: str = "127.0.0.1", port: int = 8765):
        ...
        async def mcp_smart_fetch(
            url: Annotated[str, Field(description="URL to fetch")],
            urls: Annotated[Optional[List[str]], Field(description="Multiple URLs (parallel)")] = None,
            extraction_type: Annotated[ExtendedExtractionType, Field(description="markdown|html|text|article|structured")] = "markdown",
            cache_ttl: Annotated[int, Field(description="Cache seconds. 0=fresh")] = DEFAULT_TTL,
            force_fetcher: Annotated[Optional[Literal["http", "dynamic", "stealthy"]], Field(description="Lock tier. Skips auto-escalation")] = None,
            offset: Annotated[int, Field(description="Char offset for truncated content")] = 0,
            options: Annotated[Optional[Dict[str, Any]], Field(description="css_selector, proxy, timeout, cookies, extra_headers, useragent, wait, network_idle, headless, real_chrome, respect_robots, main_content_only, use_trafilatura, solve_cloudflare, block_webrtc, hide_canvas")] = None,
        ) -> ResponseModel:
            kw = {k: v for k, v in (options or {}).items() if k in (
            ...
            return await self.smart_fetch(url=url, urls=urls, extraction_type=extraction_type,
                cache_ttl=cache_ttl, force_fetcher=force_fetcher, offset=offset, **kw)

        async def mcp_open_session(
            session_type: Annotated[SessionType, Field(description="dynamic|stealthy")],
            headless: Annotated[bool, Field(description="Headless mode")] = True,
            options: Annotated[Optional[Dict[str, Any]], Field(description="proxy, timeout, cookies, extra_headers, useragent, wait, network_idle, real_chrome, hide_canvas, block_webrtc, solve_cloudflare, session_id")] = None,
        ) -> SessionCreatedModel:
            kw = {k: v for k, v in (options or {}).items() if k in (
            ...
            return await self.open_session(session_type=session_type, headless=headless, **kw)

        async def mcp_screenshot(
            url: Annotated[str, Field(description="URL to screenshot")],
            session_id: Annotated[str, Field(description="Open session ID")],
            options: Annotated[Optional[Dict[str, Any]], Field(description="full_page, image_type, quality, wait, wait_selector, network_idle, timeout")] = None,
        ) -> List[ImageContent | TextContent]:
            kw = {k: v for k, v in (options or {}).items() if k in (
                "full_page", "image_type", "quality", "wait", "wait_selector", "network_idle", "timeout",
            )}
            return await self.screenshot(url=url, session_id=session_id, **kw)

        async def mcp_smart_search(
            query: Annotated[str, Field(description="Search query")],
            options: Annotated[Optional[Dict[str, Any]], Field(description="max_results, cache_ttl, api_key")] = None,
        ) -> SearchResponseModel:
            kw = {k: v for k, v in (options or {}).items() if k in ("max_results", "cache_ttl", "api_key")}
            return await self.smart_search(query=query, **kw)

        # ── Tool Registration ──────────────────────────────────────

        # Session management
        server.add_tool(mcp_open_session, title="open_session",
            description="Open a browser session for screenshot reuse. Returns session_id.",
            annotations=ToolAnnotations(readOnlyHint=False, destructiveHint=False, idempotentHint=False, openWorldHint=False),
            structured_output=True)
        server.add_tool(self.close_session, title="close_session",
            description="Close a browser session. Frees resources.",
            annotations=ToolAnnotations(readOnlyHint=False, destructiveHint=True, idempotentHint=True, openWorldHint=False),
            structured_output=True)
        server.add_tool(self.list_sessions, title="list_sessions",
            description="List open browser sessions with IDs, types, ages.",
            annotations=ToolAnnotations(readOnlyHint=True, idempotentHint=True, openWorldHint=False),
            structured_output=True)

        # Main fetch tool
        server.add_tool(mcp_smart_fetch, title="smart_fetch",
            description=(
                "Fetch any URL. Auto HTTP→browser→stealth escalation for bot protection. "
                "Bulk: pass urls list. Truncated? Use offset to continue. "
                "Fresh fetch: cache_ttl=0. Force tier: force_fetcher=http/dynamic/stealthy."
            ),
            annotations=ToolAnnotations(readOnlyHint=True, idempotentHint=True, openWorldHint=True),
            structured_output=True)

        # Screenshot
        server.add_tool(mcp_screenshot, title="screenshot",
            description="Screenshot a URL. Requires open_session first.",
            annotations=ToolAnnotations(readOnlyHint=True, idempotentHint=True, openWorldHint=True))

        # Search
        server.add_tool(mcp_smart_search, title="smart_search",
            description=(
                "Search the web via TinyFish API. "
                "Free key: https://agent.tinyfish.ai/sign-up?ref=v1.dXNlcl8zRGVtcWxON25nSU5aSkRzU0NLQXNtT1BVUXk.VU_44hCD2lp3YXGzJWIy3mwwsHQ8xr1TbnmQQrUNDj4 "
        ...
        server.add_tool(self.cache_clear, title="cache_clear",
            description="Clear fetch cache. Default=expired only. all=true to wipe all.",
            annotations=ToolAnnotations(readOnlyHint=False, destructiveHint=True, idempotentHint=True, openWorldHint=False),
            structured_output=True)

        # Version
        server.add_tool(self.version, title="version",
            description="Check Hound version + update availability.",
            annotations=ToolAnnotations(readOnlyHint=True, idempotentHint=True, openWorldHint=False),
            structured_output=True)
